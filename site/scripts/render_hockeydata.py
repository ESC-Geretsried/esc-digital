...
def player_card(row):
    image = str(row.get("image", "")).strip() or "images/placeholders/player.png"
    media = f'<img src="{escape(public_url(image), quote=True)}" alt="{escape(row["name"], quote=True)}" loading="lazy">'
    return (
        ...
        '<span>Position: ' + escape({"Tor": "T", "Verteidigung": "V", "Sturm": "S"}.get(str(row.get("group", "")), "")) + '</span>'
...
news_html = "".join(
    ...
    f'<a href="{escape(public_url(row["path"]), quote=True)}">Weiterlesen →</a></article>'
    for row in team["news"]
)

next_home = team.get("next_home_game") or {}
next_home_html = ""
if next_home.get("verified") is True and all(str(next_home.get(key, "")).strip() for key in ("date", "time", "opponent")):
    next_home_html = (
        '<section class="next-home-game" aria-label="Nächstes bestätigtes Heimspiel"><div class="shell">'
        '<strong>Nächstes Heimspiel</strong><span>' + escape(str(next_home["date"])) + ' · '
        + escape(str(next_home["time"])) + ' · ' + escape(str(next_home["opponent"])) + '</span></div></section>'
    )
